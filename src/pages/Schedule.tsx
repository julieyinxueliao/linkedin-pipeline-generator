...
import { Sparkles, PenSquare, CheckCircle2, RotateCcw, AlertCircle } from 'lucide-react';
...
const Calendar = () => {
  ...
  if (!brief) {
    return (
      <div className="p-8 max-w-2xl mx-auto text-center space-y-4">
        ...
        <h2 className="text-xl font-bold">Finish your Strategy Brief first</h2>
        <p className="text-muted-foreground text-sm">The calendar generates from your brief.</p>
        <Button variant="linkedin" onClick={() => navigate('/onboarding')}>Go to onboarding</Button>
      </div>
    );
  }
  ...
  const target = PRESET_MIX[brief.preset];
      ...
      <div className="flex items-start justify-between mb-8 gap-4">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-1">
            {target.label} preset · {calendar.slots.length} slots · {calendar.weeks} weeks
          </p>
        </div>
                    ...
                    <div className="flex-1 min-w-0 space-y-2">
                      ...
                      <div className="flex flex-wrap gap-1.5 items-center">
                        ...
                        <Select value={slot.archetypeId} onValueChange={(v) => updateSlot(slot.id, { archetypeId: v, archetypeName: ARCHETYPES.find((a) => a.id === v)?.name || '' })}>
                          <SelectTrigger className="h-6 w-auto text-[10px] gap-1 px-2 py-0 bg-muted border-0">
                            <SelectValue />
                          </SelectTrigger>
                          <SelectContent>
